...
ssh $SERVER "
  ...
  # 启动新服务
  cd $REMOTE_DIR
  nohup pnpm start -p $PORT > $REMOTE_DIR/viewer.log 2>&1 &
  echo '服务已启动，PID:' \$!
  sleep 3
"
...
if ssh $SERVER "ss -tlnp 2>/dev/null | grep -q :$PORT"; then
  echo "服务启动成功！"
  echo ""
  ssh $SERVER "tail -20 $REMOTE_DIR/viewer.log"
  echo ""
  echo "========================================================"
  echo "部署完成！"
  echo "访问地址: http://8.155.175.166:$PORT"
  echo "查看日志: ssh $SERVER 'tail -f $REMOTE_DIR/viewer.log'"
  echo "========================================================"
else
  echo "错误：服务启动失败！"
  echo "查看日志: ssh $SERVER 'cat $REMOTE_DIR/viewer.log'"
  exit 1
fi
